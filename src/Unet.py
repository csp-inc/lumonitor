...
class Unet(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        encoder_4 = self.encoder_block_2(x)
        x = nn.MaxPool2d(2)(encoder_4)
        x = self.center(x)
        decoder_4 = self.decoder_block_4(x)
        x = self.post_decoder_4(torch.cat(decoder_4, encoder_4))
        decoder_3 = self.decoder_block_3(x)
        x = self.post_decoder_3(torch.cat(decoder_3, encoder_3))
        decoder_2 = self.decoder_block_2(x)
        x = self.post_decoder_2(torch.cat(decoder_2, encoder_2))
        decoder_1 = self.decoder_block_1(x)
        x = self.post_decoder_1(torch.cat(decoder_1, encoder_1))
        decoder_0 = self.decoder_block_0(x)
        x = self.post_decoder_0(torch.cat(decoder_0, encoder_0))
